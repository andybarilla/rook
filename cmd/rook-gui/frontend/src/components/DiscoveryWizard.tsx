...
export function DiscoveryWizard({ onClose, onComplete }: DiscoveryWizardProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-rook-bg border border-rook-border w-[500px] max-h-[80vh] overflow-auto">
          ...
          {status === 'input' && (
            <>
              ...
              <input type="text" value={path} onChange={e => setPath(e.target.value)}
                ...
                className="w-full bg-rook-input text-rook-text border border-rook-border px-3 py-2 text-sm mb-3"
                onKeyDown={e => e.key === 'Enter' && handleDiscover()} />
              <button onClick={handleDiscover}
                className="bg-rook-active text-white px-4 py-2 text-sm w-full">
                Discover & Add
              </button>
            </>
          )}
          ...
          {status === 'done' && result && (
            <>
              ...
              {result.services && Object.keys(result.services).length > 0 && (
                <div className="space-y-1 mb-4">
                  {Object.entries(result.services).map(([name, svc]: [string, any]) => (
                    <div key={name} className="bg-rook-card px-3 py-2 text-xs">
              ...
              <button onClick={() => { onComplete(); onClose() }}
                className="bg-rook-active text-white px-4 py-2 text-sm w-full">Done</button>
            </>
          )}
          {status === 'error' && (
            <>
