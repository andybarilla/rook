...
Push a git tag matching `v*` (e.g., `v0.1.0`). The workflow runs only on tag push, not on branch push or PR.
...
**Job 2: `install-script`**
- Runs on: `ubuntu-latest`
- Steps:
  1. `actions/checkout@v4`
  2. `softprops/action-gh-release@v2` to upload `install.sh` to the same release
...
**Behavior:**
1. Detect OS (`uname -s` -> linux, darwin) and arch (`uname -m` -> x86_64/amd64, arm64/aarch64)
2. Map to archive name: `rook_<version>_<os>_<arch>.tar.gz`
3. Fetch latest release tag from `https://api.github.com/repos/andybarilla/rook/releases/latest`
4. Download archive, extract `rook` binary to `$ROOK_INSTALL_DIR` (default: `~/.local/bin`)
5. `chmod +x` the binary
6. Warn if install dir is not in `$PATH`
...
Example: `rook_0.1.0_linux_amd64.tar.gz`
